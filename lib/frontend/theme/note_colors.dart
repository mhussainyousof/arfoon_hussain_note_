import 'package:flutter/material.dart';

class AppColors {
  static const primaryBlue = Color(0xFF0081C8); 
  static const background = Colors.white;       
  static const greyText = Color(0xFFA2A2A2);    
  static const chipSelected = Colors.black;     
  static const chipUnselected = Color(0xFFE0E0E0);
  
  static const List<Color> noteColors = [
    Color(0XFF00A894), // Green - First position (index 0)
    Color(0XFFFF7E56), // Orange - Second position (index 1) 
    Color(0XFF0081C8), // Blue - Third position (index 2) - same as primaryBlue
  ];
}